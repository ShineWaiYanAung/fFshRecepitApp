  ...
  Future<void>  showAddToCartDilog(FishName itemName) async{
    showDialog(
      ...
      return AlertDialog(
        ...
        titleTextStyle: TextStyle(fontWeight: FontWeight.w500,color: AppColor.textColor,fontSize:30),
        content: TextField(
          ...
          Row(
            children: [
              TextButton(onPressed: (){
                ///Data Adding
                if(dataController.text.isNotEmpty&& RegExp(r'^[0-9]+$').hasMatch(dataController.text)){
                  setState(() {
                ...
                else if(!RegExp(r'^[0-9]+$').hasMatch(dataController.text)) {
                  setState(() {
                    dataController.text = '' ;
                  });
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text("ဂနန်းပဲထည့်ပေးပါ"),
                    ),
                  );
                }
              },
                  child: Text('မှတ်ရန်',style: TextStyle(color: AppColor.textColor,fontWeight: FontWeight.bold),),),
              TextButton(onPressed: (){
                Navigator.of(context).pop();
                setState(() {
                  isPressed = !isPressed;
                  ...
                  child: Text('ပြန်သွားရန်',style: TextStyle(color: AppColor.textColor),),)
            ],
          )
        ],
      );
